...
class AlgorithmManager(IAlgorithmManager):
    ...
    def _initialize_algorithms(self) -> None:
        for config in self._video_config:
            ...
            try:
                algorithm = AlgorithmFactory.create(algorithm_type, algorithm_config)
    ...
    def _process_video_stream(self, idx: int) -> None:
        ...
        while self._running:
            try:
                ...
                # Skip duplicate frames (shmsrc re-delivers the last buffer when no new frame arrived)
                ...
                if frame_count % 30 == 0:  # Log every 30 frames
                    self._logger.log(
                        ConstStrings.LOG_NAME_DEBUG, 
                        f"Video {idx}: Processed frame {frame_count} with algorithm {algorithm.__class__.__name__ if algorithm else 'None'} | FPS: {current_fps:.1f}"
                        )
                frame_count += 1

                # Write the latest processed frame to a shared path the GUI can read
                video_id = self._video_config[idx].get('video_id', idx + 1)
                output_path = f"/app/logs/stream_{video_id}.jpg"
                try:
                    cv2.imwrite(output_path, processed_frame)
                except Exception as e:
                    self._logger.log(ConstStrings.LOG_NAME_DEBUG, f"Error saving frame for video {video_id}: {e}")
